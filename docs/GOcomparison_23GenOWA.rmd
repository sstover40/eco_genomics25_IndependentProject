---
title: "GOcomparison_23GenOWA_RBdata"
...
From Brennan's reciprocal transplant experiment. Focusing on Ambient in Ambient control (AAAA), Ambient in OWA (HHAA) for 23 generations
sampleid:	    
AAAA_F3_REP1	
AAAA_F3_REP2	          
AAAA_F3_REP3		          
...
2. Observe how their expression changes after 23 gen of stress
3. Expand to other TE regulator GO terms 
4. determine if any are enriched after 23 generations of thermal stress 
```{r}
library(cluster)
...
counts_F3 <- read.table("recip_trans_GE/DGE_counts_F3.txt", header=TRUE, row.names=1)
head(counts_F3)
dim(counts_F3)

counts_F3_round <- round(counts_F3) # bc DESeq2 doesn't like decimals (and Salmon outputs data with decimals)
head(counts_F3_round)
colnames(counts_F3_round) <- sub("^(.*?R[0-9]).*", "\\1", colnames(counts_F3_round))
 #changing column names to match metadata table 


#import the sample description table:
metadata_F3 <- read.delim("recip_trans_GE/transplant_exp_metadata_F3.txt", header=TRUE, stringsAsFactors = TRUE, row.names=1)
head(metadata_F3)
```
...
dds <- DESeqDataSetFromMatrix(countData = counts_F3_round, colData=metadata_F3, 
                              design= ~ treatment)
...
#Ambient vs. 3Gen OWA
res_Av3OWA <- results(dds, contrast=c("group", "AAAA", "HHAA"), alpha = 0.05)
res_Av3OWA <- res_Av3OWA[order(res_Av3OWA$padj),] #sort
res_Av3OWA <- res_Av3OWA[!is.na(res_Av3OWA$padj),]

#Ambient vs. 23Gen OWA
res_Av23OWA <- results(dds, contrast=c("group", "AAAA", "HHHH"), alpha = 0.05)
res_Av23OWA <- res_Av23OWA[order(res_Av23OWA$padj),] #sort
res_Av23OWA <- res_Av23OWA[!is.na(res_Av23OWA$padj),]
...
ggsave("GE_byTreatment23OWA.png", GE_byTreatmentOWA, width=8, heigh=6, dpi=300)

```
Only 2 genes are in the DESeq object. Same genes that were found in the one gen of heat stress 
DN134741 in HHAA resembles HHHH
DN1171175 in HHAA = higher change than in AAAA or AAHH
...
TE_GO_terms <- c(
  ...
  "GO:0003697", "GO:0004386"  #got rid of the last Term --> too broad
)
...
```
Are these genes enriched after 3 generation of stress?
```{r}
#results from DESeq object (3 Generations OWA)
#convert to a df
res_df23 <- as.data.frame(res_Av23OWA)
res_df23$gene <- rownames(res_df23)

# Select significant DE genes
sig_genes23 <- res_df23 %>%
...
Enrichment_TE23 <- enricher(
  gene = sig_genes23,
  TERM2GENE = term2gene %>% filter(GO %in% TE_GO_terms),
  pAdjustMethod = "BH",
  minGSSize = 1,     # allow very small GO terms
  maxGSSize = 5000 #allow big if needed 
)

#view results 
Enrichment_TE23@result

#check
TE_term2gene <- term2gene %>% filter(GO %in% TE_GO_terms)
nrow(TE_term2gene)

TE_genes_in_DE <- intersect(sig_genes23, TE_term2gene$geneID)
length(TE_genes_in_DE)

```
No TE enriched genes also in DESeq object 

```{r}
#Comparing to WHOLE-GO enrichment: do any TE genes show up here?  
Enrichment_full23 <- enricher(sig_genes23, TERM2GENE = term2gene)
head(Enrichment_full23@result)

full_results23 <- Enrichment_full23@result

# Filter for TE-related GO terms
TE_in_full23 <- full_results23 %>%
  dplyr::filter(ID %in% TE_GO_terms)

# View the table
TE_in_full23

```
No enriched GO terms that contain TE genes 

GSEA (Gene Set Enrichment Analysis)
```{r}

# Create ranked list by log2FoldChange
geneList23 <- res_df23$log2FoldChange
names(geneList23) <- res_df23$gene

# Sort decreasing: most upregulated at top
geneList23 <- sort(geneList23, decreasing = TRUE)
...
gsea_res23 <- GSEA(geneList23,
                 TERM2GENE = term2gene,
                 pAdjustMethod = "BH",
                 verbose = FALSE)


gsea_TE23 <- gsea_res23@result %>%
  filter(ID %in% TE_GO_terms) %>%
  arrange(p.adjust)

gsea_TE23
```
no GO terms collectively downregulated or upregulated. No GO terms to visualize.  

```{r}
#visualize: 1 GO term
library(enrichplot)

# Ensure the term exists in results
if("GO:0004386" %in% gsea_res3@result$ID){
  gseaplot2(gsea_res3, geneSetID = "GO:0003697", title = "GSEA: DNA Transposition")
}

# Pick a specific TE GO term to plot, e.g., GO:0032196
library(enrichplot)

# Extract leading-edge genes for all TE GO terms
library(tidyr)

gsea_TE3_genes <- gsea_TE3 %>%
...
```
Visualizing: For multiple GSEA terms (if there are multiple terms in the GSEA)
```{r}

# Filter for TE terms that exist in the GSEA results
TE3_terms_in_gsea <- TE_GO_terms[TE_GO_terms %in% gsea_res3@result$ID]

if(length(TE3_terms_in_gsea) == 0){
  stop("No TE GO terms found in GSEA results.")
}

for(go in TE3_terms_in_gsea){
  # Get the description for title
  go_desc3 <- gsea_res3@result %>% filter(ID == go) %>% pull(Description)
  
  # Plot the enrichment curve
  print(
    gseaplot2(gsea_res3, geneSetID = go, title = paste("GSEA:", go_desc3))
  )
}

# Extract NES and FDR for TE terms
TE3_summary <- gsea_res3@result %>%
  dplyr::filter(ID %in% TE3_terms_in_gsea) %>%
