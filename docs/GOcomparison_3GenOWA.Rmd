...
#ggsave("GE_byTreatmentOWA.png", GE_byTreatmentOWA, width=8, heigh=6, dpi=300)
...
#convert to a df
res_df3 <- as.data.frame(res_Av3OWA)
...
#view results 
Enrichment_TE3@result

```
1 gene out of 25 the could be mapped to the list of GO terms is in the list of DEGs but it is not significantly enriched 
One GO term (GO:0004386) is enriched but not significantly, this indicated helicase activity, may not necessarily be connected to TE regulation 

GSEA (Gene Set Enrichment Analysis)
```{r}

# Create ranked list by log2FoldChange
geneList3 <- res_df3$log2FoldChange
names(geneList3) <- res_df3$gene
...
gsea_TE3 <- gsea_res3@result %>%
  filter(ID %in% TE_GO_terms) %>%
  arrange(p.adjust)

gsea_TE3
```
The genes in GO:0003697 are all collectively downregulated (25 genes)
individual genes not significant in DESeq2 but there is a significant coordinated trend 

Similar to GO:0004386, 55 genes are downregulated. They are enriched in the DESeq object but not significantly. Likely helicase activity outside of TE regulation.
They are also collectively downregulated 
...
for(go in TE3_terms_in_gsea){
  ...
  enrichment_curve <- gseaplot2(gsea_res3, geneSetID = go, title = paste("GSEA:", go_desc3))
  
  # Save as PNG
  ggsave(
    filename = paste0("GSEA_", go, ".png"),  # concatenate "GSEA_" + GO ID + ".png"
    plot = enrichment_curve,
    width = 10,
    height = 5,
    dpi = 300
  )
